...
class StickyTestAdapter(context: Context, private val gravity: Int)
    : StickyAdapter<StickyTestAdapter.GroupViewHolder, StickyTestAdapter.ChildViewHolder>() {
    ...
    override fun onCreateGroupViewHolder(parent: ViewGroup, viewType: Int): GroupViewHolder {
        val layoutResId = when (gravity) {
            StickyParams.CENTER -> R.layout.item_group_center
            StickyParams.END -> R.layout.item_group_right
            else -> R.layout.item_group_left
        }
